...
public class MyChannelWidget implements IChannelType {
    ...
    @Override
    public void bindViewHolder(final ChannelAdapter.ChannelViewHolder holder, int position, final ComicSource data) {
        final MyChannelHeaderViewHolder myHolder = (MyChannelHeaderViewHolder) holder;
        myHolder.mChannelTitleTv.setText(data.getTitle());
        // 设置文字大小，通过判断tab中的文字长度，如果有4或者4个字以上则为16sp大小
        int textSize = data.getTitle().length() >= 4 ? 14 : 16;
